...
function matchLetter(myLetter,myId,wordLen){
    let counter =0;
    ...
    inputVal.addEventListener('keyup',function(event){
        ...
        console.log(myId,gotValue,myLetter,counter);
    });
}

function verifyWord(wordLen){
    if(counter == wordLen){
        var auxStr= "word"+String(idx);
    ...
    }else{console.log("some letters are wrong",idx);}
}
async function build_boxes(){
    ...
    for(var word of data){
        ...
        for(var thisLtr of word){
            ...
            //console.log(gotId,thisLtr);
            matchLetter(thisLtr,gotId);
            jdx = jdx + 1;
        }
        //verifyWord(word.length);
        idx = idx + 1;
    }//for idx
}
